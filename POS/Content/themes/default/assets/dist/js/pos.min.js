                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < data.Data.SaleItems.length; i++) {
                        ...
                        obj = {
                            ...
                            Product_code: data.Data.SaleItems[i].Product_code,
        ...
        $.each(spositems, function () {
          ...
          debugger
            ...
            if (KeyId == item_id && $('#TaxClassification').val() =="") {
                ...
                //    var pds = ds.split('%');
                //    if (!isNaN(pds[0])) {
                //        Item_dis = formatDecimal(parseFloat((net_price * parseFloat(pds[0])) / 100), 4);
            ...
            tr_service_html = '<td></td><td colspan="2"><div class="form-group"><label>Assign To</label><div class="input-group" style="position: relative"><input type="text" class="form-control" id="AssignId" value="' +
                item.Title +
                '" /><button class="btn btn-primary bank-btn" type="button" data-toggle="modal" data-target="#AssignUserModal"><i class="fa fa-search"></i></button></div></div></td><td><input type="text" class="form-control datepicker" onchange="GetFromDate(this.value)" placeholder="From" value="' + item.FromDate + '" /></td><td><input type="text" class="form-control datepicker" onchange="GetToDate(this.value)" placeholder="To" value="' + item.ToDate + '" /></td>';
            tr_html =
...
$(document).ready(function () {
        ...
        response: function (event, ui) { 
            ...
            //else if (ui.content.length == 1 && ui.content[0] && ui.content[0].Product_id != 0)
            {
                var data = {
                    ...
                    Quantity: 1,
                    Product_code: ui.content[0].Product_code,
    ...
    $('#submit-sale').click(function () {
        ...
        var result = {
            "Customer_name": $('#CustomerId').find('option:selected').text(),
            "Customer_id": $("#CustomerId").val(),
            "Paid": $("#amount").val(),
            "Note": $('#hold_ref').val(),
                ...
                success: function (data) {
                    ...
                    bootbox.confirm("Do you want to print order?", function (result) {
                        if (result == true) {
                            ...
                            window.open("" + base_url + "/POS/PrintOrder?Id=" + data.Data.Id, '_blank');
                        }
                        if (result == false) {
                            window.location.href = base_url + "/POS/Index";
                        }
    ...
    //$('.load_suspended').on('click', function (e) {
        ...
        $.ajax({
            type: 'get',
            url: base_url + '/SuspendedSale/GetById/' + Id,
            contentType: 'application/json',
            dataType: 'json',
            data: {},
            success: function (data) {
