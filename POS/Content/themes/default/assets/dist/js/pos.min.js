function add_invoice_item(item) {
    ...
    var item_id = Settings.item_addition == 1 ? item.ItemId : item.ItemId;
    if (spositems[item_id]) {
        spositems[item_id].Qty = parseFloat(spositems[item_id].Qty) + parseFloat(item.Qty || 1);
    } else {
...
function loadItems() {
    ...
    if (get('spositems')) {
        ...
        $.each(spositems, function () {
            ...
            var product_id = item.ItemId,
                item_type = item.type,
                //item_tax_method = parseFloat(item.row.TaxClassificationNo),
                combo_items = false,
                item_qty = item.Qty,
                item_aqty = parseFloat(item_qty),
                item_type = '',
                item_ds = 0,
                item_code = item.ItemBarcode,
                item_name = item.ItemName;//.replace(/"/g, '&#034;').replace(/'/g, '&#039;');
            ...
            var pr_tax = parseFloat(item.taxRate/100),
                pr_tax_val = 0;
            ...
            product_tax += item_qty * net_price * 0.15;
            //product_tax += formatDecimal(pr_tax_val * item_qty, 4);

            var row_no = new Date().getTime();
            var newTr = $('<tr id="' + row_no + '" class="' + item_id + '" data-item-id="' + item.ItemId + '" data-id="' + item.ItemId + '"></tr>');
            tr_html =
...
$(document).ready(function () {
    ...
    $('#add_item').autocomplete({
        ...
        response: function (event, ui) { 
            ...
            if ($(this).val().length >= 16 && ui.content[0] && ui.content[0].ItemId == 0) {
                bootbox.alert("no match found", function () {
                    $('#add_item').focus();
                });
                $(this).val('');
            } else if (ui.content.length == 1 && ui.content[0] && ui.content[0].ItemId != 0) {
