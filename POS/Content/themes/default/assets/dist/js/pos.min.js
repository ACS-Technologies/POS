...
$(document).ready(function () {
    ...
    $('#submit-sale').click(function () {
        ...
        if ($("#PaymentMethod").val() == "" && $("#AccidentId").val()=="" ) {
            $("#PaymentMethod").focus();
            bootbox.alert("Please Choose Payment Method");
        }
        else {
            if ($("#AccidentId").val() != "") {
                $(".EnduranceRatio .select2-selection").css('border', '');
                $(".InsuranceCompanies .select2-selection").css('border', '');
                $("#CustomerRatio").css('border', '');
                $("#InsuranceRatio").css('border', '');
                if ($("#EnduranceRatio").val() == "0") {
                    $(".EnduranceRatio .select2-selection").css('border', '1px solid red');
                    return false;
                }
                else {
                    if ($("#EnduranceRatio").val() == 2 && $("#InsuranceCompanies").val()=="") {
                        $(".InsuranceCompanies .select2-selection").css('border', '1px solid red');
                        return false;
                    }
                    if ($("#EnduranceRatio").val() == 3 && $("#InsuranceCompanies").val() == "") {
                        $(".InsuranceCompanies .select2-selection").css('border', '1px solid red');
                        return false;
                    }
                    else {
                        if ($("#CustomerRatio").val() == 0 && $("#EnduranceRatio").val() == 3) {
                            $("#CustomerRatio").css('border', '1px solid red');
                            return false;
                        }
                        else if ($("#InsuranceRatio").val() == 0 && $("#EnduranceRatio").val() == 3) {
                            $("#InsuranceRatio").css('border', '1px solid red');
                            return false;
                        }
                    }
                }
            ...
            if ($('#AccidentId').val()!="") {
                Accident = {
                    ...
                    "EnduranceRatio": $("#EnduranceRatio").val(),
                    "InsuranceCompanies": $("#InsuranceCompanies").val(),
                    "CustomerRatio": $("#EnduranceRatio").val() == 1 ? 100: $("#CustomerRatio").val(),
                    "InsuranceRatio": $("#InsuranceRatio").val(),
                    "CustomerValue": $("#EnduranceRatio").val() == 1 ? parseFloat($("#amount").val()): $("#CustomerValue").val(),
                    "InsuranceValue": $("#InsuranceValue").val(),
                    'DiscountInsurance': $("#EnduranceRatio").val() != 1 ? parseFloat($("#amount").val()) - (parseFloat($("#CustomerValue").val()) + parseFloat($("#InsuranceValue").val())) : 0,
                    'Movment': $("#Movment").val()
                }
            }
            var result = {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (IsWorkShop == true) {
                    $('#Vehicle_id').val(data.Data.Vehicle_id).trigger('change');
                    $('#Vehicle_id').val(data.Data.Vehicle_id).trigger('change');
                    if (data.Data.AccidentId != 0) {
                        $("#Ratio").css("display", "block");
                        $("#paid").css("display", "none");
                        $('#AccidentId').val(data.Data.AccidentId); 
                        
                            $.ajax({
                                type: 'get',
                                url: base_url + '/POS/AccidentInfo_Get?Id=' + $('#AccidentId').val(),
                                contentType: 'application/json',
                                dataType: 'json',
                                data: {},
                                success: function (data) {
                                    ...
                                    if (data.AgreementId != 0) {
                                        ...
                                        $('#Movment').val(true);
                                        $('#EnduranceRatio').val(2).trigger('change');
                                        $("#EnduranceRatio").attr('disabled', 'disabled');
                                    }
...
function SendKeyId(_KeyId) {
    var userid = 116;//spositems[KeyToUserId].ToUserId;
    var name = "";

    for (var i = 0; i < Workshops.length; i++) {
        if (name == "") {
            for (var j = 0; j < Workshops[i].BranchWorkshop.length; j++) {
                if (name == "") {
                    if (userid == Workshops[i].BranchWorkshop[j].UserId) {
                        name = Workshops[i].BranchWorkshop[j].PrimaryName;
                        break;
                    }
                    

                    for (var z = 0; z < Workshops[i].SubBranchWorkshop.length; z++) {
                        if (Workshops[i].SubBranchWorkshop[z].ParentId == Workshops[i].BranchWorkshop[j].Id && Workshops[i].SubBranchWorkshop[z].UserId == userid) {
                            if (name == "") {
                                name = Workshops[i].SubBranchWorkshop[z].PrimaryName;
                                break;
                            }
                            else {
                                break;
...
$('#EnduranceRatio').change(function (e) {
    
    if ($('#EnduranceRatio').val() == 3) {
        $(".modal-dialog .info").css("display", "block");    
        $(".modal-dialog #Companies").css("display", "block");   
        $(".modal-dialog .Value").css("display", "block");
    }
    else {
        $(".modal-dialog .info").css("display", "none");
        $(".modal-dialog #Companies").css("display", "none");
        $(".modal-dialog .Value").css("display", "none");  
    }
    if ($('#EnduranceRatio').val() == 2) {
        $("#Companies").css("display", "block");
    }
    $('#InsuranceRatio').val(round(0, 2));
    $('#CustomerRatio').val(round(0, 2));
    $('#InsuranceValue').val(round(0, 2));
    $('#CustomerValue').val(round(0, 2));
});
var DiscountPercentage = 0;
var DiscountMethod = null;
var InsuranceCompany = null;
var InsuranceCalculation = null;
var customerValue = 0;
var InsuranceValue = 0;
var customerEquation = null;
var InsuranceEquation = null;
$('#InsuranceCompanies').change(function (e) {
    
    if ($('#InsuranceCompanies').val() !="") {
        InsuranceCompany = InsuranceCompanies.find(x => x.Id == $('#InsuranceCompanies').val());
        DiscountPercentage = InsuranceCompany.DiscountPercentage;
        DiscountMethod = InsuranceCompany.DiscountMethod;

        if ($('#EnduranceRatio').val() == 2) {
            $(".modal-dialog .Value").css("display", "block");
            $('#CustomerValue').val(round(0, 2));
            var amount = parseFloat($("#amount").val());
            $('#InsuranceValue').val(round(amount - (amount * DiscountPercentage / 100), 2));
        }
        else {
            $('#InsuranceRatio').val(round(0, 2));
            $('#CustomerRatio').val(round(0, 2));
...
$('#CustomerRatio,#InsuranceRatio').on('input', function () {
    
    if (parseInt($('#CustomerRatio').val()) > 100 && ($('#InsuranceRatio').val() == "0" || $('#InsuranceRatio').val() != "0")) {
        $('#CustomerRatio').val(round(100, 2));
        $('#InsuranceRatio').val(round(0, 2));
    }
    if (parseInt($('#InsuranceRatio').val()) > 100&& ($('#CustomerRatio').val() == "0" || $('#CustomerRatio').val() != "0")) {
        $('#CustomerRatio').val(round(100, 2));
        $('#InsuranceRatio').val(round(0, 2));
    }
    if (parseInt($('#CustomerRatio').val()) > 0 && ($('#InsuranceRatio').val() == "0" || $('#InsuranceRatio').val() != "0")) {
        $('#InsuranceRatio').val(round( 100 - parseInt($('#CustomerRatio').val()),2));
    }
    if (parseInt($('#InsuranceRatio').val()) > 0 && ($('#CustomerRatio').val() == "0" || $('#CustomerRatio').val() != "0")) {
        $('#CustomerRatio').val(round( 100 - parseInt($('#InsuranceRatio').val()),2));
    }
...
function Calculate() {
    ...
    if ($('#CustomerRatio') != 0 && $('#CustomerRatio') != "" && $('#InsuranceRatio').val() != 0 && $('#InsuranceRatio').val() != "" && $('#InsuranceCompanies').val() > 0) {

        InsuranceCompany = InsuranceDiscountCalculation.find(x => x.Id == DiscountMethod);
        customerEquation = InsuranceCompany.CustomerEquation;
        InsuranceEquation = InsuranceCompany.InsuranceCompanyEquation;
        var customerSubtitue = customerEquation.replaceAll('[Amount]', $("#amount").val()).replaceAll('[Discount Percentage]', DiscountPercentage/100).replaceAll('[Customer Percentage]', $('#CustomerRatio').val()/100).replaceAll('[Insurance Company Percentage]', $('#InsuranceRatio').val()/100);
        var companysubitute = InsuranceEquation.replaceAll('[Amount]', $("#amount").val()).replaceAll('[Discount Percentage]', DiscountPercentage/100).replaceAll('[Customer Percentage]', $('#CustomerRatio').val()/100).replaceAll('[Insurance Company Percentage]', $('#InsuranceRatio').val()/100);
        $('#CustomerValue').val(round(evil(customerSubtitue), 2));
        $('#InsuranceValue').val(round(evil(companysubitute), 2));
    }
